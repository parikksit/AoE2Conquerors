...
public class TownCenter extends Unit {	
	...
	public void retrieve(String civName, InputStream in, String tag) {
		InputStream is = in;
		Log.i("Parikksit", "Just inside Retrieve "+tag);
		try {
			Log.i("Parikksit", "Inside Try "+tag );
		...
		public void startElement(String uri, String localName,String qName, 
			...
			Log.i("Parikksit", "Inside startElement "+tcTag);
		
			if(qName.equalsIgnoreCase("building"))
			{
				Log.i("Parikksit", "inside EqualsIgnoreCase(buildG) " +tcTag);
				inBuilding = true;
				building = true;
				...
				if(id==5)
				{
					Log.i("Parikksit", "Inside if(id==5) " +tcTag);
			...
			Log.i("Parikksit", "before going to cost node "+tcTag);
			if(inBuilding && id==5){
				Log.i("Parikksit", "before going to cost node");
				if(qName.equalsIgnoreCase("cost")){

					Log.i("Parikksit", "Inside COST node "+tcTag);
				...
				Log.i("Parikksit", "before going to Unit node");
				if(qName.equalsIgnoreCase("unit")){
					Log.i("Parikksit", "Inside Unit Node");
		...
		// END ELEMENT here		
	     ...
	     Log.i("Parikksit", "Now, going to parse the doc");
	     //InputStream xmlFile = context.getApplicationContext().getAssets().open("civilizations.xml");
	     
	     saxParser.parse(is, handler);
	     Log.i("Parikksit", "after parsing");
	     } catch (Exception e) {
	       e.printStackTrace();
	       Log.i("Parikksit", e.toString());
	     }
